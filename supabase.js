...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNiZWFieGxzbmpjY2xjY2FzcWRpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODIxMDQ0MDksImV4cCI6MjA5NzY4MDQwOX0.zsoUh0tu5U-6exwK2USJv7jFvEfqPunYlfGULIy8rLk';
...
async function _doSync() {
  ...
  const app = _getAPP();
  if (!app) { _err('APP tidak ditemukan'); return; }
...
async function _syncTable(table, uid, items) {
  ...
  /* Upsert semua */
  const { error: upsertErr } = await _sb.from(table).upsert(mapped, { onConflict: 'id' });
  if (upsertErr) { _err(`Upsert ${table} gagal`, upsertErr); return; }
...
function _mapToDb(table, uid, item) {
  const base = { id: item.id, user_id: uid, updated_at: new Date().toISOString() };

  switch (table) {
    case 'quests':
      return { ...base,
        judul: item.judul, deskripsi: item.deskripsi,
        kategori: item.kategori, hari: item.hari,
        exp: item.exp, tipe: item.tipe,
        selesai: item.selesai, tanggal: item.tanggal
      };
    case 'inventory':
      return { ...base,
        gambar: item.gambar, nama: item.nama,
        harga: item.harga, tanggal_beli: item.tanggalBeli,
        kondisi: item.kondisi, catatan: item.catatan
      };
    case 'wishlist':
      return { ...base,
        gambar: item.gambar, nama: item.nama,
        spesifikasi: item.spesifikasi, harga: item.harga,
        grup: item.grup, status: item.status,
        tanggal_beli: item.tanggalBeli
      };
    case 'goals':
      return { ...base,
        judul: item.judul, kategori: item.kategori,
        deskripsi: item.deskripsi, selesai: item.selesai
      };
    case 'savings':
      return { ...base,
        nama: item.nama, target: item.target,
        terkumpul: item.terkumpul, deadline: item.deadline
      };
    case 'finance_accounts':
      return { ...base,
        nama: item.nama, ikon: item.ikon, saldo: item.saldo
      };
    case 'transactions':
      return { ...base,
        tipe: item.tipe, nominal: item.nominal,
        kategori: item.kategori, akun: item.akun,
        tanggal: item.tanggal, catatan: item.catatan
      };
    case 'activity_logs':
      return { ...base,
        tipe: item.tipe, teks: item.teks,
        waktu: item.waktu ?? new Date().toISOString()
      };
    case 'ai_chats':
      return { ...base,
        judul: item.judul, pesan: item.pesan ?? []
      };
    case 'sosial':
      return { ...base,
        tipe: item.tipe, deskripsi: item.deskripsi, tanggal: item.tanggal
      };
    default:
      return { ...base, ...item };
  }
...
async function loadData() {
  if (!_currentUser) { _log('loadData: belum login, skip.'); return; }
  const app = _getAPP();
  if (!app) { _err('APP tidak ditemukan'); return; }
  ...
  try {
    const [
      profile, quests, inventory, wishlist,
      goals, savings, accounts, transactions,
      actLog, aiChats, sosial
    ] = await Promise.all([
      ...
      _sb.from('activity_logs').select('*').eq('user_id', uid).order('waktu', { ascending: false }).limit(200),
    ...
    if (profile.data) {
      ...
      app.profil        = { nama: p.nama, panggilan: p.panggilan, umur: p.umur, lahir: p.lahir, kota: p.kota, deskripsi: p.deskripsi, foto: p.foto };
      app.level         = p.level ?? 1;
      app.exp           = p.exp ?? 0;
      app.totalExp      = p.total_exp ?? 0;
      app.darkMode      = p.dark_mode ?? false;
      app.stats         = p.stats ?? { pengetahuan:0, kesehatan:0, uang:0, sosial:0 };
    ...
    if (inventory.data?.length)    app.inventory   = inventory.data.map(_mapFromDb_inventory);
    if (wishlist.data?.length)     app.wishlist    = wishlist.data.map(_mapFromDb_wishlist);
    if (goals.data?.length)        app.goals       = goals.data.map(_mapFromDb_goals);
    if (savings.data?.length)      app.savings     = savings.data.map(_mapFromDb_savings);
    if (accounts.data?.length)     app.accounts    = accounts.data.map(_mapFromDb_accounts);
    if (transactions.data?.length) app.transactions = transactions.data.map(_mapFromDb_transactions);
    if (actLog.data?.length)       app.activityLog  = actLog.data.map(_mapFromDb_actlog);
...
/* ============================================================
   ...
   ============================================================ */
const _mapFromDb_quest        = r => ({ id: r.id, judul: r.judul, deskripsi: r.deskripsi, kategori: r.kategori, hari: r.hari, exp: r.exp, tipe: r.tipe, selesai: r.selesai, tanggal: r.tanggal });
const _mapFromDb_inventory    = r => ({ id: r.id, gambar: r.gambar, nama: r.nama, harga: r.harga, tanggalBeli: r.tanggal_beli, kondisi: r.kondisi, catatan: r.catatan });
const _mapFromDb_wishlist     = r => ({ id: r.id, gambar: r.gambar, nama: r.nama, spesifikasi: r.spesifikasi, harga: r.harga, grup: r.grup, status: r.status, tanggalBeli: r.tanggal_beli });
const _mapFromDb_goals        = r => ({ id: r.id, judul: r.judul, kategori: r.kategori, deskripsi: r.deskripsi, selesai: r.selesai });
const _mapFromDb_savings      = r => ({ id: r.id, nama: r.nama, target: r.target, terkumpul: r.terkumpul, deadline: r.deadline });
const _mapFromDb_accounts     = r => ({ id: r.id, nama: r.nama, ikon: r.ikon, saldo: r.saldo });
const _mapFromDb_transactions = r => ({ id: r.id, tipe: r.tipe, nominal: r.nominal, kategori: r.kategori, akun: r.akun, tanggal: r.tanggal, catatan: r.catatan });
const _mapFromDb_actlog       = r => ({ id: r.id, tipe: r.tipe, teks: r.teks, waktu: r.waktu });
const _mapFromDb_aichat       = r => ({ id: r.id, judul: r.judul, pesan: r.pesan ?? [] });
const _mapFromDb_sosial       = r => ({ id: r.id, tipe: r.tipe, deskripsi: r.deskripsi, tanggal: r.tanggal });
